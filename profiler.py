...
Fills every slot by prefilling n_slots requests (B=1, q_len=PROMPT_LEN each),
then profiles a handful of decode steps at full batch. Either phase can be
profiled on its own; decode always needs the prefills to run first, they just
go unrecorded when only decode is asked for. Writes one chrome trace per
profiled phase; open them at ui.perfetto.dev.
...
def fill_slots(
    ...
    """One prefill per step until every slot is full, exactly as the engine does.

    Every request is fresh (pos=0) and B=1 reads only its own slot, so each step
    is the same work no matter how full the pool is -- steady-state prefill.
    ...
    for i in range(n_slots):
        ...
        llm.scheduler.add(Request(id=i, prompt=prompt, max_new_tokens=10_000))
...
def main() -> None:
    ...
    # Size the pool to the run, as main.py does. Not cosmetic: the captured decode
    # step attends over the whole slot_len window whatever is actually cached, so a
    # slot_len that disagrees with the workload silently rescales every decode
    # number. Deriving it also makes a slot overflow unreachable.
    # +1: prefill writes columns [0, PROMPT_LEN), then the capture step and each of
    # the STEPS profiled steps write one more.
    engine_config = replace(EngineConfig(), slot_len=PROMPT_LEN + STEPS + 1)
    model = load_model(model_config, engine_config)
    llm = LLMEngine(engine_config)
    cache = SlotCache(model_config, engine_config)
    ...
    if args.decode:
        # Capture the CUDA graph before the profiler opens. The first decode step
        # builds DecodeGraph and runs three side-stream warmup forwards; none of
        # that is a decode step, and WAIT alone was keeping it out of the trace.
